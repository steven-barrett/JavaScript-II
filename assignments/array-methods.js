...
runners.forEach(person => {
    fullName.push(`First name: ${person.first_name}, Last name: ${person.last_name}`)
})
...
let ticketPriceTotal = runners.reduce((acc, person) => acc + person.donation, 0);
...
// Problem 1: Get the persons full name and email address
let fullNameEmail = [];
runners.forEach(person => {
    fullNameEmail.push(`${person.first_name}, ${person.last_name} email is: ${person.email}`);
})
console.log(fullNameEmail);

// Problem 2: Get the first name and what company they work for
let firstNameAndCompany = [];
runners.forEach(person => {
    firstNameAndCompany.push(`${person.first_name} works for:  ${person.company_name}`);
})
console.log(firstNameAndCompany);

// Problem 3: Return all the people that donated less than 100$
let smallDonations = runners.filter(person => person.donation < 100)
